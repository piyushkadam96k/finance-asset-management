...
async function loadTransactions() {
    ...
    try {
        ...
        table.innerHTML = "<thead><tr><th>ID</th><th>Reference</th><th>Type</th><th>Asset</th><th>Amount</th><th>Date</th><th>Actions</th></tr></thead><tbody></tbody>";
        const tbody = table.querySelector('tbody');
...
// Download report PDF (client-side generation)
...
function getReportCSS() {
    return `
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        @media print { body { padding: 0; background: white; } .report { box-shadow: none; border: none; padding: 20px; } }
    `;
}
...
function generateProfitLossHTML(data) {
    ...
    breakdown.forEach((item, i) => {
        ...
        const plSign = pl >= 0 ? '+' : '';
        rowsHTML += `<tr><td>${i + 1}</td><td>${item.assetName || 'N/A'}</td><td class="text-right ${plClass}">${plSign}₹${pl.toLocaleString('en-IN')}</td><td class="text-right ${plClass}">${plSign}${pct}%</td></tr>`;
    });
    return `<!DOCTYPE html><html><head><title>Profit & Loss Report - FAMS</title><style>${getReportCSS()}</style></head><body>
        <div class="report"><div class="header"><h1>🏦 FAMS</h1><h2>Profit & Loss Report</h2><div class="date">Generated on ${new Date().toLocaleDateString('en-IN', { year: 'numeric', month: 'long', day: 'numeric' })}</div></div>
        <div class="summary-grid"><div class="summary-card"><div class="label">Total Profit</div><div class="value profit">₹${totalProfit.toLocaleString('en-IN')}</div></div><div class="summary-card"><div class="label">Total Loss</div><div class="value loss">₹${totalLoss.toLocaleString('en-IN')}</div></div><div class="summary-card"><div class="label">Net P&L</div><div class="value ${net >= 0 ? 'profit' : 'loss'}">${net >= 0 ? '+' : ''}₹${net.toLocaleString('en-IN')}</div></div></div>
        <h3 class="section-title">Asset-wise Breakdown</h3><table><thead><tr><th>#</th><th>Asset Name</th><th class="text-right">Profit / Loss</th><th class="text-right">Change %</th></tr></thead><tbody>${rowsHTML}</tbody></table>
        <div class="footer"><p>This is a computer-generated report.</p><p style="margin-top: 8px;">Finance Asset Management System (FAMS) — ${new Date().toLocaleString()}</p></div></div>
        </body></html>`;
}
...
function generateAssetUtilizationHTML(data) {
    ...
    let rowsHTML = '';
    items.forEach((item, i) => { rowsHTML += `<tr><td>${i + 1}</td><td>${item.name || 'N/A'}</td><td>${item.category || 'N/A'}</td><td class="text-right">${item.utilization || 'N/A'}</td><td>${item.status || 'N/A'}</td></tr>`; });
    return `<!DOCTYPE html><html><head><title>Asset Utilization Report - FAMS</title><style>${getReportCSS()}</style></head><body>
        <div class="report"><div class="header"><h1>🏦 FAMS</h1><h2>Asset Utilization Report</h2><div class="date">Generated on ${new Date().toLocaleDateString('en-IN', { year: 'numeric', month: 'long', day: 'numeric' })}</div></div>
        <div class="summary-grid"><div class="summary-card"><div class="label">Total Assets</div><div class="value">${items.length}</div></div><div class="summary-card"><div class="label">Active Assets</div><div class="value profit">${items.filter(i => i.status === 'ACTIVE').length}</div></div></div>
        <h3 class="section-title">Asset Details</h3><table><thead><tr><th>#</th><th>Asset Name</th><th>Category</th><th class="text-right">Utilization</th><th>Status</th></tr></thead><tbody>${rowsHTML}</tbody></table>
        <div class="footer"><p>This is a computer-generated report.</p><p style="margin-top: 8px;">Finance Asset Management System (FAMS) — ${new Date().toLocaleString()}</p></div></div>
        </body></html>`;
}

function generateBalanceSheetHTML(data) {
    return `<!DOCTYPE html><html><head><title>Balance Sheet - FAMS</title><style>${getReportCSS()}</style></head><body>
        <div class="report"><div class="header"><h1>🏦 FAMS</h1><h2>Balance Sheet</h2><div class="date">As of ${new Date().toLocaleDateString('en-IN', { year: 'numeric', month: 'long', day: 'numeric' })}</div></div>
        <div class="summary-grid"><div class="summary-card"><div class="label">Total Assets</div><div class="value profit">₹${(data.totalAssets || 0).toLocaleString('en-IN')}</div></div><div class="summary-card"><div class="label">Total Liabilities</div><div class="value loss">₹${(data.totalLiabilities || 0).toLocaleString('en-IN')}</div></div><div class="summary-card"><div class="label">Net Worth</div><div class="value" style="color: #1e3a8a;">₹${(data.netWorth || 0).toLocaleString('en-IN')}</div></div></div>
        <h3 class="section-title">Summary</h3><table><thead><tr><th>Item</th><th class="text-right">Amount (₹)</th></tr></thead><tbody>
        ...
        <tr style="background: #f1f5f9;"><td><strong>Net Worth</strong></td><td class="text-right" style="font-size: 1.1rem; font-weight: 700; color: #1e3a8a;">₹${(data.netWorth || 0).toLocaleString('en-IN')}</td></tr>
        </tbody></table>
        <div class="footer"><p>This is a computer-generated report.</p><p style="margin-top: 8px;">Finance Asset Management System (FAMS) — ${new Date().toLocaleString()}</p></div></div>
        </body></html>`;
}
...
async function editTransaction(txId) {
    try {
        showLoading();
        // Fetch specific transaction from list
